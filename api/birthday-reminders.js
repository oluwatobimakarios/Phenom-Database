  ...
  try {
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: ADMIN_EMAILS,
        subject: `🎂 ${birthdaysToday.length} birthday${birthdaysToday.length > 1 ? 's' : ''} today`,
        html: `
          <div style="font-family:sans-serif;font-size:15px;color:#181614;">
            <h2 style="margin-bottom:4px;">Today's Birthdays 🎉</h2>
    ...
    return res.status(200).json({
      message: `Email sent for ${birthdaysToday.length} birthday(s).`,
